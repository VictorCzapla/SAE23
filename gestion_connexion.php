...
// Redirect to manager page if already logged in as manager
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check credentials against the Batiment table
    $requete = mysqli_query($connexion, "SELECT * FROM Batiment WHERE login_gestionnaire = '$identifiant' AND mdp_gestionnaire = '$password'");
    ...
    if ($user) {
        // Store user info in session and redirect to manager panel
...
<html lang="fr">
 ...
 <body>
  ...
  <header>
   <h1> Gestion : Page de Connexion</h1>
   <nav class="navbar1">
    <ul>
  ...
  <section class="box-noir">
    ...
    <form name="connexion_gestion" action="gestion_connexion.php" method="post" class="form-admin">
        <fieldset class="form-fieldset-clean">
            <p>
                <label for="identifiant">Identifiant Gestionnaire : </label>
